...
def get_pi_number(user):
    """
    Generates the user number based on the current Coordinator.
    Args:
...
def create_hostapd(user):
    """
    Install and  Generates a hostapd configuration file based on the current Coordinator.
...
def create_dnsmasq(user):
    """
    Install and  Generates a dnsmasq configuration file based on the current Coordinator.
...
def create_dhcpcd(user):
    """
    Generates a dhcpcd configuration file based on the current Coordinator.
...
def adjust_wpa_range(user):
    ...
    ssid_range = range(2, 199) # The range of available IPs that your Coordinator will be able to use
    pi_number = get_pi_number(user)
...
def adjust_rc_file():
    ...
    to fix issues whit IP addressing after restart of the RPi as Coordinator.
...
if user_input.lower() == "y" or user_input.lower() == "yes":
    ...
    print("Reboot cancelled.\nPlease Reboot your system before you change mode of the device.")
